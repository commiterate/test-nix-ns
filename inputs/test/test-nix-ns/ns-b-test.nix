...
// {
  # Equivalent to:
  # - `(require ["test-nix-ns.ns-b" :as ns-b])`
  # - `import "test-nix-ns.ns-b" as ns-b`
  ns-b = U."test-nix-ns.ns-b";
};
...
let
  test-strawberry =
    assert ns-b.strawberry == 20;
    null;
  test-cake =
    assert ns-b.cake == 2;
    null;
  test-strawberry-cake =
    assert ns-b.strawberry-cake == 22;
    null;
in
...
{
  inherit test-strawberry test-cake test-strawberry-cake;
}
